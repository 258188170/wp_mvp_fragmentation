    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if ("POST".equals(method)) {
            ...
            if (request.body() instanceof FormBody) {
                ...
                Timber.tag(TAG).d(String.format("发送请求 %s on %s %n%s %nRequestParams:{%s}",
                        request.url(), chain.connection(), request.headers(), sb.toString()));
            }
        } else {
            Timber.tag(TAG).d(String.format("发送请求 %s on %s%n%s",
        ...
        Timber.tag(TAG).d(String.format("接收响应: [%s] %n返回json:【%s】 %.1fms %n%s",
